...
<div class="panel panel-info" style="width: 97%; margin:auto">
	...
	<div class="panel-body">
		<c:if test="${packageObj.id ne '0'}">
			<s:form action="updatePackage" role="form" enctype="multipart/form-data" theme="simple">			
		...
		<c:if test="${packageObj.id eq '0'}">
			<s:form action="addPackage" role="form" enctype="multipart/form-data" theme="simple">			
				<c:if test="${message != null}">
					<div class="form-group has-error message">
						<label class="control-label" for="error"><s:property value="message" escapeHtml="false"/></label>
					</div>
			...
			<table class="table table-striped table-bordered table-hover">
				...
				<s:iterator value="packageList" status="status">
					<tr>
						...
						<td>${title}</td>
						<td>${titleDetail}</td>
						<td><c:if test="${itemStatus eq '1'}">Yes</c:if><c:if test="${itemStatus ne '1'}">No</c:if></td>
						<td>${colorSuffix}</td>
						<td><img src="${backgroundUrl}" style="width: 200px; height: 150px;"></td>
						<td><a href="<%=request.getContextPath()%>/viewPackageDetail?id=${id}">view</a></td>
						<td><a onclick="return confirm('Are you sure you want to Delete?');" href="<%=request.getContextPath()%>/deletePackage?id=${id}">delete</a></td>
					</tr>
				</s:iterator>
			</tbody>
			</table>
