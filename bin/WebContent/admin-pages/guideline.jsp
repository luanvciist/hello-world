...
<div class="panel panel-info" style="width: 97%; margin:auto">
	...
	<div class="panel-body">
 		<c:if test="${guideline.id ne '0'}">
			<s:form action="updateGuideline" role="form" enctype="multipart/form-data" theme="simple">			
 		...
 		<c:if test="${guideline.id eq '0'}">
			<s:form action="addGuideline" role="form" enctype="multipart/form-data" theme="simple">			
				<c:if test="${message != null}">
					<div class="form-group has-error message">
						<label class="control-label" for="error"><s:property value="message" escapeHtml="false"/></label>
					</div>
			...
			<table class="table table-striped table-bordered table-hover">
				...
				<s:iterator value="guidelineList" status="status">
					<tr>
						...
						<td>${content}</td>
						<td>${href}</td>
						<td><c:if test="${popupStatus eq '1'}">Yes</c:if><c:if test="${popupStatus ne '1'}">No</c:if></td>
						<td><img src="${src}" style="width: 100px; height: 70px;"></td>
						<td><a href="<%=request.getContextPath()%>/viewGuidelineDetail?id=${id}">view</a></td>
						<td><a onclick="return confirm('Are you sure you want to Delete?');" href="<%=request.getContextPath()%>/deleteGuideline?id=${id}">delete</a></td>
					</tr>
				</s:iterator>
			</tbody>
			</table>
